...
const CreateForm = () => {
    const [fId, fundraiserName] = window.location.href.replace('http://localhost:3000/update/blog/', '').split('/')
    ...
    async function handleSubmit(e) {
        ...
        try {
            const data = await fetch('http://localhost:9999/api/blog/edit', {
                method: 'PUT',
            ...
            <div className={styles.loginWrapper}>
                ...
                <form className={styles.loginForm}
                    ...
                    <Input name="img-link"
                        ...
                        placeholder="Edit blog image link" />
                    <p className={styles.error}> {image.errorMsg} </p>

                    <textarea placeholder="Edit blog content"
                ...
                <div className={styles.new}>
                    <p>Made a mistake?</p>
                    <Link to={'/account-blogs'} className={styles.register}>Go back</Link>
                </div>
            </div>
        </div>
    )
